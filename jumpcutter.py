...
parser.add_argument('-f', '--force', default=False,
                    action='store_true', help='Overwrite output_file without asking')
...
parser.add_argument('-sr', '--sample_rate', type=float, default=44100,
                    help="sample rate of the input and output videos")
parser.add_argument('-fr', '--frame_rate', type=float,
...
parser.add_argument('-p', '--preset', type=str, default="medium",
                    help="A preset is a collection of options that will provide a certain encoding speed to compression ratio. See https://trac.ffmpeg.org/wiki/Encode/H.264")
parser.add_argument('-crf', '--crf', type=int, default=23,
                    help="Constant Rate Factor (CRF). Lower value - better quality but large filesize. See https://trac.ffmpeg.org/wiki/Encode/H.264")
parser.add_argument('-alg', '--stretch_algorithm', type=str, default="wsola",
                    help="Sound stretching algorithm. 'phasevocoder' is best in general, but sounds phasy. 'wsola' may have a bit of wobble, but sounds better in many cases.")
parser.add_argument('-a', '--audio_only', default=False,
                    action='store_true', help="outputs an audio file")
parser.add_argument('-edl', '--edl', default=False, action='store_true',
                    help='EDL export option. (Supports only cuts off)')
...
TEMP_FOLDER = "temp_frames_"+str(math.floor(time()*1000))
...
if not (AUDIO_ONLY or EDL):
    ...
    if rc.returncode != 0:
        deletePathAndExit(
            TEMP_FOLDER, "The input file doesn't have any video. Try --audio_only", rc.returncode)

command = ["ffmpeg", "-i", INPUT_FILE, "-ab", "160k", "-ac", "2",
           "-ar", str(SAMPLE_RATE), "-vn", TEMP_FOLDER+"/audio.wav"]
rc = subprocess.run(command)
if rc.returncode != 0:
    deletePathAndExit(
        TEMP_FOLDER, "The input file doesn't have any sound.", rc.returncode)
...
if frameRate is None:
    try:
        ...
        if AUDIO_ONLY:
            ...
            deletePathAndExit(
                TEMP_FOLDER, "Couldn't detect a framerate.", rc.returncode)
...
for chunk in chunks:
    ...
    if leng < AUDIO_FADE_ENVELOPE_SIZE:
        ...
        for i in range(endPointer-AUDIO_FADE_ENVELOPE_SIZE, endPointer):
            outputAudioData[i][0] *= (1-mask[i -
                                             endPointer+AUDIO_FADE_ENVELOPE_SIZE])
            outputAudioData[i][1] *= (1-mask[i -
                                             endPointer+AUDIO_FADE_ENVELOPE_SIZE])
    if not AUDIO_ONLY:
        ...
        for outputFrame in range(startOutputFrame, endOutputFrame):
            ...
            if outputFrame % 1000 == 999:
                print(str(inputFrame + 1) + "/" + str(audioFrameCount) +
                      " frames processed.", end="\r", flush=True)
            if didItWork:
